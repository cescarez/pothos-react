...
const Rating = ({ baseURL, request, currentUserData, maxRating }) => {
    ...
    const isRateable = () => {
        ...
        //can rate if date of service has passed (not today)
        const serviceDateHasPassed = today > Moment(request.date_of_service).add(1, 'days').startOf('day')
        return (
            serviceDateHasPassed && (request.status === 'confirmed')
        )
    }
